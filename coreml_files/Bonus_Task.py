...
def create_asymmetric_noise(labels, rate):
    noisy_labels = labels.copy()
    
    truck_indices = np.where(labels == 9)[0]
    ...
    truck_flip = np.random.choice(truck_indices, int(len(truck_indices) * rate), replace=False)
    bird_flip = np.random.choice(bird_indices, int(len(bird_indices) * rate), replace=False)
    deer_flip = np.random.choice(deer_indices, int(len(deer_indices) * rate), replace=False)
    cat_flip = np.random.choice(cat_indices, int(len(cat_indices) * rate), replace=False)
    dog_flip = np.random.choice(dog_indices, int(len(dog_indices) * rate), replace=False)
    
    noisy_labels[truck_flip] = 1
    noisy_labels[bird_flip] = 0
    noisy_labels[deer_flip] = 7
    noisy_labels[cat_flip] = 5
    noisy_labels[dog_flip] = 3
...
def main():
    ...
    for rate in asym_noise_rates:
        print(f"\n=== Training with asymmetric noise rate η = {rate} ===")
        
        train_labels = np.array(full_train_data.targets)
        noisy_labels = create_asymmetric_noise(train_labels, rate)
        ...
        for loss_name, loss_fn in loss_functions.items():
            ...
            plot_title = f"{loss_name} - Asymmetric Noise Rate {rate}"
            save_path = f"{results_dir}/{loss_name}_asym_{rate}.png"
